...
class Trilinear_interpolation_cuda(torch.autograd.Function):
    @staticmethod
    def forward(ctx, feats, points):        # ctx -- context
    ...
    def backward(ctx, dL_dfeat_interp):
        ...
        return dL_dfeats,None     # 多少Inputs 就返回多少，补'None'
...
if __name__ == "__main__":
    ...
    points = torch.rand(N, 3, device='cuda')*2-1    #[-1,1]
    ...
    loss.backward()
    torch.cuda.synchronize()
    print('     python bw time', time.time()-t, 's')
